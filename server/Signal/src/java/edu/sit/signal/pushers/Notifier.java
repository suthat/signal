...
public class Notifier implements Runnable {
    ...
    public Notifier(HashMap notification) {
        ...
        ampqNotifier = new AMQPNotifier(config.rabbitMQHost);
    ...
    public void run() {
        ...
        if (this.isNotifyTo(platforms, NDLEnum.Scope.PLATFORM_IOS.getSring())) {
            ...
            App app = new App();
            app.connect();
            HashMap apns = app.getAPNs(notification.get("uuid").toString());
            app.disconnect();
            
            if(apns != null){
                String pathToCer = config.apnsCerKeyPath + "" + apns.get("certificate").toString();
                String pathToP12 = config.apnsCerKeyPath + "" + apns.get("p12key").toString();
        ...
        if (this.isNotifyTo(platforms, NDLEnum.Scope.PLATFORM_ANDROID.getSring())) {
            ...
            App app = new App();
            app.connect();
            HashMap gcm = app.getCGM(notification.get("uuid").toString());
            app.disconnect();
            if(gcm != null){
                ...
                //System.out.println("Exception: No Google APIs Key");
            }
        }
    }
